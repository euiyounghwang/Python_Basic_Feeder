...
class SQL_Manager:
    ...
    def read_sql_elastic_post(self, conn, sql):
        """
        DB Record를 elastic_post 방식으로 색인
        :param conn:
        :param sql:
        :return:
        """
        ...
        if df_all.shape[0] > 0:
            ...
            # print(df_all.keys().tolist())
            # print('PD -> ', df_all['KEY'][0])
            # ---

            import ES_Basic_Feeder.Basic_Feeder as POST

            header = {'Content-Type': 'application/x-ndjson', 'Authorization': 'Basic xxxx=='}
            ...
            if df_all.shape[0] > 0:
                ...
                for loop in range(0, len(df_all._get_values)):
                    ...
                    docs.append(
                        {
                            '_op_type': 'index',
                            '_index': POST.INDICS_NAME,
                            '_type': '_doc',
                            # '_id': 'new_id_' + str(df_all.get('KEY')[loop]),
                            '_id': 'new_id_' + str(loop),
                            '_source' : dict(each_row_columns)
                            # '_source': {
                            #     column: str(df_all.get(column)[loop]),
        ...
        # POST Buffer Remains
        # ---
        # if len(docs) > 0: POST.Elastic_INSERT_BULK(docs)
    ...
    def read_sql_http_post(self, conn, sql):
        ...
        if df_all.shape[0] > 0:
            ...
            if df_all.shape[0] > 0:
                ...
                for loop in range(0, len(df_all._get_values)):
                    ...
                    POST.Http_INSERT_BULK(url, header, loop, docs)
        ...
        # POST Buffer Remains
        # ---
        # if len(docs) > 0: POST.Http_INSERT_BULK(url, header, loop, docs)
...
class Run():
    ...
    def Invoke(self):
        ...
        try:
            ...
            # Elastic POST
            # --
            # SQL_Manager().read_sql_elastic_post(OBJ_CONN, Manager.get_SQL())
